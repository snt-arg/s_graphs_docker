...
if(!class_exists('BE_Import_Customizer_Option')) {
	final class BE_Import_Customizer_Option extends WP_Customize_Setting {
...
if(!class_exists('BECustomizerImport')) {
	final class BECustomizerImport {
		...
		static private function _import( $wp_customize, $file ) 
		{
			...
			// Setup internal vars.
			$cei_error	 = false;
			//$template	 = get_template();comment for tatsu standalone
			...
			if ( isset( $data['options'] ) ) {
				...
				foreach ( $data['options'] as $option_key => $option_value ) {
					if( !in_array( $option_key, self::$core_options ) ) {
						$option = new BE_Import_Customizer_Option( $wp_customize, $option_key, array(
							'default'		=> '',
							'type'			=> 'option',
